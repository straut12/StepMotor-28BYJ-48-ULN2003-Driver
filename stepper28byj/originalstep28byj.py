...
if __name__ == "__main__":
    ...
    incomingD = {"delay":[1.6,1.6], "speed":[2,2], "mode":[0,0], "inverse":[False,True], "step":[2038, 2038], "startstep":[0,0]}
    ...
    for i in range(len(mach.stepper)):          # Setup each stepper motor
        ...
        targetstep.append(291)         # Increment stepping target. Will be updated with node-red gui.
        interval.append(97)         # Default interval for publishing updates. Will be updated with node-red gui. Do not want too many messages sent. Only updates if 4076 % interval == 2
    ...
    def motors(command):   # command comes from node-red GUI
        ...
        for i in range(len(mach.stepper)):   # Loop thru each stepper
            ...
            if not command["inverse"][i]: # Normal rotation pattern. speed 3/4=rot1(CW). speed 0/1=rot0 (CCW). 
            ...
            elif command["inverse"][i]:  # Inverse rotation pattern. speed 3/4=rot0(CCW). speed 0/1=rot1 (CW). 
                mach.stepper[i].speed[0] = mach.stepper[i].coils["FarrOUT"][1]
                mach.stepper[i].speed[1] = mach.stepper[i].coils["HarrOUT"][1]
                mach.stepper[i].speed[3] = mach.stepper[i].coils["HarrOUT"][0]
                mach.stepper[i].speed[4] = mach.stepper[i].coils["FarrOUT"][0]
            ...
            if (abs(mach.stepper[i].step) > FULLREVOLUTION) or stepreset:  # If hit full revolution reset the step counter. If want to step past full revolution would need to later add a 'not startstepping'
                ...
                if stepreset:
                    ...
                    stepreset = False
        sleep(float(command["delay"][i])/1000)  # delay can be updated from node-red gui. Needs optimal setting for the motors.
